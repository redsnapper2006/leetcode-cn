...
func smallestK(arr []int, k int) []int {
	...
	var qs func(arr []int, k int)
	qs = func(arr []int, k int) {
		pivot := arr[0]
		s, e := 0, len(arr)-1
		for s <= e {
			for s < len(arr) && arr[s] <= pivot {
				s++
			}
			for e >= 0 && arr[e] > pivot {
				e--
